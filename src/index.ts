...
try {
  ...
  // 如果不是JSON格式，按逗号分隔处理
  nicknames = MEOW_NICKNAMES.split(",")
    .map((name) => name.trim())
...
server.registerTool(
  "send_notification",
  {
    ...
    inputSchema: {
      message: z
        .string()
        .describe("Required notification message content (plain text or HTML)"),
      url: z
        .string()
        .optional()
      ...
      title: z
        ...
        .optional()
        .describe(
          "Optional title for the notification (defaults to 'meow-notifier')",
        ),
      msgType: z
        .enum(["text", "html"])
        .optional()
        .describe(
          "Message display type: 'text' (default, plain text) or 'html' (render HTML format in App)",
        ),
      htmlHeight: z
        .number()
        .optional()
        .describe(
          "HTML message height in pixels (only effective when msgType='html', default 200)",
        ),
    },
  },
  ...
  "send_notification_guide",
  {
    ...
    argsSchema: {
      message: z
        .string()
        .optional()
        .describe("Optional message to customize the prompt"),
    },
  },
